...
<h2>Decision framework</h2><p>Use a four-part filter: fit, evidence, risk, and next step.</p><table><thead><tr><th>Filter</th><th>The question it asks</th></tr></thead><tbody><tr><td>Fit</td><td>Does the resource match your situation?</td></tr><tr><td>Evidence</td><td>What is the claim based on?</td></tr><tr><td>Risk</td><td>What could go wrong if you misunderstand the topic?</td></tr><tr><td>Next step</td><td>Do you need a checklist, a consult, a quote, or a qualified professional?</td></tr></tbody></table>{''.join('<p>'+html.escape(x)+'</p>' for x in body[5:10])}
<h2>Checklist</h2><ul>{''.join('<li>'+html.escape(x)+'</li>' for x in checklist)}</ul>
{product_section}<h2>Useful citation</h2><p>For readers comparing this category, <a href="{html.escape(target_url)}"{rel_attr(target_url)}>{html.escape(anchor)}</a> may be a relevant next resource when it matches the situation. The link is included because it belongs in the topic area, not because every reader needs it.</p>
<h2>FAQ</h2>{faq_html}
